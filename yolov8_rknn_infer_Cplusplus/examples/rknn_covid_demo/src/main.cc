...
int main(int argc, char** argv)
{
  ...
  while (true) {
	  ...
	  int img_width = 720;
	  int img_height = 1280;
//	  cv::Mat frame = cv::imread("/home/firefly/rknn/test/" + std::to_string(idx) + ".jpg", 1);
	  ...
	  if (!frame.data)
	 {
	  ...
	  cv::imwrite("./" + std::to_string(idx++) + "_org.jpg", frame);
...
//	 cv::cvtColor(src_image, img, cv::COLOR_BGR2RGB);
      ...
      cv::imwrite("/home/firefly/rknn/" + std::to_string(idx++) + ".jpg", frame);
  }
  //关闭程序时，释放内存
  ret = ReleaseEnv();
  return 0;
